...
class AgentContrarian(AgentBaseModel):
    """
    Agent that goes against market trends. Buys if the price has decreased by 1% or more,
    ...
    def __init__(self, available_capital: float, owned_gpus: int, total_agents: int, distribution: Dict[str, int]):
        """
        Initializes a contrarian agent with financial resources and market data.
